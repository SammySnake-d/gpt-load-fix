...
type ConfigApplier struct {
	...
	// 当前应用的配置
	appliedConfigs map[uint]*OptimizerOptimalConfig
	mu             sync.RWMutex
...
type ConfigApplication struct {
	...
	Config      *OptimizerOptimalConfig `json:"config"`
...
func NewConfigApplier(poolManager *PoolManager) *ConfigApplier {
	return &ConfigApplier{
		poolManager:        poolManager,
		appliedConfigs:     make(map[uint]*OptimizerOptimalConfig),
		applicationHistory: make([]*ConfigApplication, 0),
	}
}

// ApplyOptimalConfig 应用最优配置到指定分组
func (ca *ConfigApplier) ApplyOptimalConfig(groupID uint, config *OptimizerOptimalConfig) error {
	ca.mu.Lock()
	defer ca.mu.Unlock()
...
func (ca *ConfigApplier) applyMemoryPoolConfig(pool *MemoryLayeredPool, config *OptimizerOptimalConfig) error {
	// 更新分片存储配置
...
func (ca *ConfigApplier) ApplyToAllGroups(config *OptimizerOptimalConfig) error {
	// 获取所有分组
...
func (ca *ConfigApplier) RollbackConfig(groupID uint) error {
	...
	// 查找上一个成功的配置
	var previousConfig *OptimizerOptimalConfig
...
func (ca *ConfigApplier) recordApplication(groupID uint, config *OptimizerOptimalConfig, success bool, errorMsg string, rollback bool) {
...
func (ca *ConfigApplier) GetAppliedConfig(groupID uint) (*OptimizerOptimalConfig, bool) {
	ca.mu.RLock()
	defer ca.mu.RUnlock()
...
// ValidateConfig 验证配置
func (ca *ConfigApplier) ValidateConfig(config *OptimizerOptimalConfig) error {
	if config == nil {
